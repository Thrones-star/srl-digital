<template>
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <h2 class="text-center my-4">Pinjam Alat</h2>
                <div class="card rounded-5" style="width: 70rem; height: 50vh; background-color: #D9D9D9;">
...
const kirimData = async () => {
    ...
    const { error } = await supabase.from("penggunaan_alat").insert([form.value])
    if (error) throw error
    else navigateTo("/alat")
}
...
const getkeperluan = async () => {
    const { data, error } = await supabase.from("keperluan_alat").select("*")
